...
private void guardar(){
    // Validar que no haya espacios en blanco en los campos
    if(!txt_nfactura.getText().trim().isEmpty() && !txt_codigo.getText().trim().isEmpty() && !txt_cantidad.getText().trim().isEmpty()){
        ...
        if (fechaa.compareTo(fechaActual) <= 0) {
            java.sql.Date fecha_sql = new java.sql.Date(fechaa.getTime());
            ...
            JOptionPane.showMessageDialog(this, "La fecha de entrada no puede ser futura", "Error", JOptionPane.ERROR_MESSAGE);
        }
    } else {
        JOptionPane.showMessageDialog(this, "No se permiten campos en blanco", "Error", JOptionPane.ERROR_MESSAGE);
    }
}
